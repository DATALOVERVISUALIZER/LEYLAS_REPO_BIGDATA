...
########################################GET  DATA ############################
# Data 
setwd("C:/Users/FB006366/Desktop/SPRINTS/SPRINT8/SIMGE_RISK_DATA_MODEL")
getwd()
...
test_operation_colnames<-c ("MUSTERI_NO","TC_KIMLIK_NO","MUSTERI_MESLEK","MUSTERI_SUBESI_KODU",
                             ...
                             "IZIN_POSTA","IZIN_SMS","IZIN_CM","MUSTERI_NO","VERIKAYNAK_STATU_ID","MUSTERI_TIPI","MUSTERI_YASI","MUSTERI_SEGMENT","MUSTERI_SINIFI",
                            ...
                            "ONONAY5T_RESULTTEXT","ONONAYIHT_SUBPRODUCTTYPE","KKB_SKORU","KKB_EGILIM_SKORU","KKB_SORGU_TRH","NEWLY_APPROVED","APPNO"
                             )     
...
drop.cols_test<- c ("TC_KIMLIK_NO","MUSTERI_SUBESI_KODU","MUSTERI_SUBESI_ADI",
                    ...
                    "ONONAYIHT_SUBPRODUCTTYPE","KKB_SKORU","KKB_EGILIM_SKORU","KKB_SORGU_TRH","NEWLY_APPROVED","APPNO") 
...
drop.cols2<- c( "MUSTERI_KANALI.ONLINE SAT-ARACKIRA", 
                "MUSTERI_KANALI.MIKS-Tüzel", "MUSTERI_KANALI.TELE PERFORMANCE", "MUSTERI_KANALI.BIREYSEL INTERNET" )
test <- test %>% dplyr::select(-one_of(drop.cols2))


drop.cols3<- c( "MUSTERI_KANALI.WebService")
train <- train %>% dplyr::select(-one_of(drop.cols3))
...
x_test$`MUSTERI_KANALI.ONLINE SAT-ARACKIRA`<-NULL

x_train$`MEDENI_DURUM.Bekar`<-NULL

x_train$`MUSTERI_KANALI.BIREYSEL INTERNET`<-NULL


setdiff(x_test,x_train)

x_test$MEDENI_DURUM.Bekar<-NULL
...
Model.sprint8<-cbind(test_operation, predictions)
...
write.csv(Model.sprint8, "Model.sprint8_simge.csv")
